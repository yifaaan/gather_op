...
std::vector<float> TestGatherCHW(bool is_rvv, std::vector<int> in_shape,
  ...
  if (in_shape.size() == 3) {
    ...
    input_data = convert_nhwc_to_nchw_4d(input_data, in_shape[0], in_shape[1],
                                         in_shape[2], in_shape[3], 16);

  } else if (in_shape.size() == 5) {
    input_data =
        convert_ndhwc_to_ncdhw_5d(input_data, in_shape[0], in_shape[1],
                                  in_shape[2], in_shape[3], in_shape[4], 16);
  }
  ...
  gettimeofday(&start, NULL);
  if (in_shape.size() == 3) {
    output = convert_chw_to_hwc_3d(output, 128, 128, 128, 16);
  } else if (in_shape.size() == 4) {
    // input: 128, 128, 128, 5
    // axis = 0,2
    output = convert_nhwc_to_nchw_4d(output, 128, 128, 128, 5, 16);
  } else if (in_shape.size() == 5) {
    // input: 128, 128, 128, 5, 5
    // axis = 4,3
    output = convert_ncdhw_to_ndhwc_5d(output, 5, 128, 3, 128, 128, 16);
  }
  gettimeofday(&end, NULL);
  ...
  if (in_shape.size() == 3) {
    ...
    printf("input{%2d, %2d, %2d, %2d, %2d},axis{%d},channel_%2d,CLtoNCDHW %.3f "
           ...
           axis, 16, convert_time_use, convert_time_use2, calculate_time_use,
           all_time);
  }
